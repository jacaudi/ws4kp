    ...
    containers:
      app:
        image:
          ...
          tag: v7.1.5 # x-release-please-version — pin a release; auto-bumped on each release
        env:
      ...
      ws4kp:
        containers:
          app:
            image:
              repository: ghcr.io/jacaudi/ws4kp
              tag: v7.1.5 # x-release-please-version
    service:
      app:
        controller: ws4kp
        ports:
...
- **Image tags:** pin a release (`v7.1.5`) for reproducible rollouts. The registry also publishes `:latest` (from `main`) and `:sha-<short>` for every build; all images are multi-arch (amd64 + arm64), so they run on ARM nodes too. <!-- x-release-please-version -->
