    ...
    containers:
      app:
        image:
          ...
          tag: v7.1.0 # pin a release; matches the ghcr semver tags
        env:
...
- **Image tags:** pin a release (`v7.1.0`) for reproducible rollouts. The registry also publishes `:latest` (from `main`) and `:sha-<short>` for every build; all images are multi-arch (amd64 + arm64), so they run on ARM nodes too.
